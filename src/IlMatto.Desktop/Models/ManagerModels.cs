...
public sealed class ManagerImageAttachment
{
    ...
    public static ManagerImageAttachment FromPath(string path)
    {
        ...
        return new ManagerImageAttachment
        {
            AttachmentId = Guid.NewGuid().ToString("N"),
            Path = fullPath,
            DisplayName = System.IO.Path.GetFileName(fullPath),
            MimeType = MimeTypeForExtension(System.IO.Path.GetExtension(fullPath)),
        };
    }
...
public sealed class ManagerMainAgentBinding
{
    ...
    // Null is intentionally retained for old snapshots. ManagerViewModel
    ...
    public bool TerminalSandbox { get; set; }
    // Retained for JSON compatibility; Antigravity turns are unbounded.
    public int TimeoutSeconds { get; set; }
    public string BaseUrl { get; set; } = "";
    public string ModelId { get; set; } = "";
    public string CredentialId { get; set; } = "";
    public string? SessionRef { get; set; }
    public string? SdkSessionRef { get; set; }
    public string? LegacyCliConversationId { get; set; }
    public string DisplayName => Provider == "openai_compatible"
        ? "API Manager"
        : "Antigravity CLI";
}
...
public sealed class ManagerCodingAgentBinding
{
    ...
    public string DisplayName => Provider switch { "codex" => "Codex", "antigravity" => "Antigravity", _ => "Pi" };
}
...
public partial class ManagerChatEntry : ObservableObject
{
    ...
    public void SetTransientText(string value)
    {
        Text = value ?? "";
        Segments.Clear();
        OnPropertyChanged(nameof(HasMessageBody));
...
public sealed record TaskExecutorChoice(string Provider, string DisplayName);
