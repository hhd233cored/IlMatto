...
test("manager host reports missing Antigravity without falling back to code execution", async () => {
  ...
  try {
    ...
    if (child.exitCode === null) {
      await new Promise<void>((resolve) => { const timer = setTimeout(() => { try { child.kill(); } catch { } resolve(); }, 2_000); child.once("exit", () => { clearTimeout(timer); resolve(); }); });
    }
...
async function connectWithRetry(pipePath: string): Promise<net.Socket> {
  ...
  while (Date.now() < deadline) {
    try { return await new Promise<net.Socket>((resolve, reject) => { const socket = net.createConnection(pipePath); socket.once("connect", () => resolve(socket)); socket.once("error", reject); }); }
