...
export type CodeTask = { type: "code_task"; taskId: string; userRequest: string };
 ...
 */
export function isExplicitCodingRequest(value: string): boolean {
  const text = value.trim();
  if (!text) return false;
  if (/(怎么|如何|是什么|为什么|区别|解释|说明|介绍|教程|示例|语法)/u.test(text)) return false;
...
export function createCodeTask(taskId: string, originalUserRequest: string, action: ManagerAction): CodeTask {
  if (action.action !== "delegate_code") throw new Error("Only delegate_code actions can create a CodeTask");
...
export function sanitizeCoordinatorInput(originalUserRequest: string): string {
  ...
  const lines = text.split(/\r?\n/).map((line) => {
    if (/^(?:PS\s+[^>]+>|@@\s|\+\+\+\s|---\s|[+\-]\s*[^\s])/i.test(line.trim())) return "[local diff or terminal output omitted]";
    if (/^\s*at\s+\S+.*:\d+(?::\d+)?\s*$/i.test(line)) return "[local stack trace omitted]";
