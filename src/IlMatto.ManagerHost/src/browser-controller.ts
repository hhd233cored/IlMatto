...
import { defaultBrowserPermissions, type BrowserMcpOperation, type BrowserPermissionName, type BrowserPermissions, type BrowserRequest, type BrowserState } from "./protocol.js";
...
export class BrowserController {
  ...
  private startPromise?: Promise<BrowserControllerResult>;
  private readonly sessionPermissions = new Map<string, Record<BrowserPermissionName, boolean>>();

  constructor(private readonly notify: (event: BrowserControllerEvent) => void) {}

  get currentState(): BrowserState { return this.state; }

  configureSession(sessionId: string, permissions?: BrowserPermissions): void {
    this.sessionPermissions.set(sessionId, { ...defaultBrowserPermissions, ...(permissions ?? {}) });
  }

  async handle(sessionId: string, request: BrowserRequest): Promise<BrowserControllerResult> {
    switch (request.operation) {
      ...
      default:
        ...
        if (this.state === "Stopped") await this.start(sessionId);
  ...
  async setVisibility(sessionId: string, visible: boolean): Promise<BrowserControllerResult> {
    this.ensureOwner(sessionId);
    ...
    if (starting) await this.start(sessionId);
  ...
  async humanDone(sessionId: string): Promise<BrowserControllerResult> {
    this.ensureOwner(sessionId);
    if (this.state !== "WaitingForHuman" && this.state !== "HumanControlled") return this.snapshotState(sessionId);
  ...
  approve(sessionId: string, actionId: string, approved: boolean): void {
  ...
  private async start(sessionId: string): Promise<BrowserControllerResult> {
    ...
    if (this.chrome) await this.stopProcess();
  ...
  private async startCore(sessionId: string): Promise<BrowserControllerResult> {
    this.state = "Starting";
    ...
    try {
      const endpoint = await Promise.race([
        ...
        new Promise<string>((_resolve, reject) => chrome.once("error", reject)),
      ]);
  ...
  private async stop(sessionId: string): Promise<BrowserControllerResult> {
    this.ensureOwner(sessionId);
    await this.stopProcess();
    this.state = "Stopped";
    this.ownerSessionId = undefined;
  ...
  private async authorize(sessionId: string, request: BrowserRequest): Promise<BrowserControllerResult> {
    this.ensureOwner(sessionId);
    await this.refreshHealth(sessionId);
  ...
  private pageState(sessionId: string, page?: BrowserRequest["page"]): BrowserControllerResult {
    this.ensureOwner(sessionId);
    const url = page?.url ?? "";
  ...
  private ensureOwner(sessionId: string): void {
    if (this.ownerSessionId && this.ownerSessionId !== sessionId) {
      throw new BrowserControllerError("BROWSER_LOCKED", "浏览器正在由另一个 Manager 会话使用。" );
    }
    if (!this.ownerSessionId) this.ownerSessionId = sessionId;
  ...
  private snapshotState(sessionId: string): BrowserControllerResult {
   ...
   */
  private async refreshHealth(sessionId: string): Promise<void> {
    if (this.state === "Stopped" || this.state === "Starting") return;
    const child = this.chrome;
    if (!child || child.killed || child.exitCode !== null || !this.port || !this.cdpEndpoint) return;
    let healthy = false;
  ...
  private emitState(sessionId: string, message?: string): void {
  ...
  private async showWindow(visible: boolean, maximize = false): Promise<boolean> {
...
Write-Output ($(if ($found) { "1" } else { "0" }))`;
    return await new Promise<boolean>((resolve) => execFile("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command", script], { windowsHide: true }, (error, stdout) => {
...
function resolveChromeExecutable(): string | undefined {
  ...
  if (process.platform !== "win32") return configured || "google-chrome";
  const candidates = [
...
function resolveBrowserNewTabUrl(executable: string): string {
...
async function waitForCdp(port: number, child: ReturnType<typeof spawn>): Promise<string> {
  const deadline = Date.now() + 20_000;
  let lastError = "CDP endpoint 尚未就绪。";
  while (Date.now() < deadline) {
    if (child.exitCode !== null) throw new Error(`Chrome 在 CDP 启动前退出（退出码 ${child.exitCode ?? "unknown"}）。`);
    try {
      const response = await fetch(`http://127.0.0.1:${port}/json/version`);
      if (response.ok) {
        const payload = await response.json() as { webSocketDebuggerUrl?: unknown };
        if (typeof payload.webSocketDebuggerUrl === "string" && payload.webSocketDebuggerUrl) return payload.webSocketDebuggerUrl;
...
function looksLikeHumanVerification(value: string): boolean {
...
export function isBrowserWriteOperation(operation: BrowserMcpOperation): boolean {
...
function permissionFromDetails(details?: string): BrowserPermissionName | undefined {
