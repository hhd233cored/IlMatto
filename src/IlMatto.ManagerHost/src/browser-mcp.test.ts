...
test("Browser MCP exposes the configured browser tool surface", async () => {
  ...
  await new Promise<void>((resolve, reject) => { managerPipe.once("error", reject); managerPipe.listen(pipePath, resolve); });
...
async function waitFor(predicate: () => boolean, timeoutMs = 3_000): Promise<void> {
  ...
  while (!predicate()) {
    if (Date.now() - started >= timeoutMs) throw new Error("Timed out waiting for Browser MCP response.");
