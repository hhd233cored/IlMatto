...
import type { CodexObservationOperation, ManagerClientMessage } from "./protocol.js";
...
class ManagerRpcClient {
  ...
  private onData(chunk: string): void {
    ...
    while (newline >= 0) {
      ...
      if (message.type !== "codex_observation_response" || typeof message.requestId !== "string") continue;
      const pending = this.pending.get(message.requestId);
      if (!pending) continue;
      clearTimeout(pending.timer); this.pending.delete(message.requestId);
      if (message.ok === true) pending.resolve(message.data);
      else pending.reject(Object.assign(new Error(message.error?.message ?? "Codex 观察请求失败。"), { code: message.error?.code ?? "CODEX_OBSERVATION_ERROR" }));
    }
  }
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (!operation) throw new Error(`不支持的 Codex 观察工具：${name}`);
