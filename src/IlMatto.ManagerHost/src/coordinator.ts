...
import { validateManagerAction, type MainAgentConfig, type ManagerAction, type ManagerImageAttachment } from "./protocol.js";
...
export class AntigravityCoordinator implements CoordinatorProvider {
  ...
  constructor(config: Extract<MainAgentConfig, { provider: "antigravity" }>, private readonly runtime: ManagerRuntime, private readonly sessionId = runtime.agentName) {
    this.session = new AntigravitySession(
      ...
      0,
      config.effort ?? "medium", config.model?.trim() || undefined, config.conversationId, undefined, sessionId,
    );
  }
