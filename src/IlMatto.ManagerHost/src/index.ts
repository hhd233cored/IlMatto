...
import { isAuthenticationError, probeAntigravity, AntigravitySession, type AntigravityProbe, type CoordinatorStreamEvent } from "./antigravity.js";
...
import type { AntigravityTransport, CompanionProfile, CodingAgentConfig, ManagerClientMessage, ManagerHostMessage, ManagerImageAttachment, MainAgentConfig } from "./protocol.js";
...
import { cleanupManagerRuntime, ensureUnifiedManagerRuntime, type ManagerRuntime } from "./runtime.js";
import { buildImageAwareCompanionPrompt, stageManagedImages } from "./image-staging.js";
import { CodexObservationController, type CodexObservationProgressEvent, type CodexObservationRequest } from "./codex-observation.js";
...
let activeSessionOwner: net.Socket | undefined;
...
const server = net.createServer((socket) => {
  ...
  socket.on("close", () => {
    if (activeSessionOwner !== socket) return;
...
async function handle(message: ManagerClientMessage, send: Send, socket?: net.Socket): Promise<void> {
  ...
  if (message.type === "delete_manager_session") {
    if (activeSessionStart) await activeSessionStart.catch(() => undefined);
  ...
  if (message.type === "start_manager_session") {
    ...
    if (!activeSession) {
      ...
      catch (error) { await activeSession.dispose(); activeSession = undefined; activeSessionOwner = undefined; throw error; }
  ...
  switch (message.type) {
    case "send_manager_message": await activeSession.prompt(message.text, message.attachments, message.executor, message.draftId); break;
    ...
    case "cancel_manager_turn": activeSession.cancel(); break;
...
class ManagerSession {
  ...
  async prompt(userMessage: string, attachments: ManagerImageAttachment[] = [], executor?: Extract<ManagerClientMessage, { type: "send_manager_message" }>['executor'], draftId?: string): Promise<void> {
    ...
    this.busy = true; this.cancelled = false; this.setState("responding");
    try {
      if (this.legacyCoordinator) {
        let streamed = false;
        ...
        if (!this.cancelled) this.send({ type: "manager_completed", sessionId: this.id, source: "api_manager", text: "", action: turn.action.action, final: true });
        this.busy = false; this.setState(this.cancelled ? "cancelled" : "idle");
        return;
      }
      ...
      if (!this.cancelled) this.send({ type: "manager_completed", sessionId: this.id, source: "antigravity", text: "", action: "respond", final: true });
      this.busy = false; this.setState(this.cancelled ? "cancelled" : "idle");
    } catch (error) {
      this.busy = false;
  ...
  private setState(state: Extract<ManagerHostMessage, { type: "manager_state" }>["state"]): void {
...
function buildUnifiedPrompt(userMessage: string, profile: CompanionProfile): string {
