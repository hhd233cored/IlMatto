...
import type { AgentToolRequest, AntigravityTransport, CompanionMemoryRequest, CompanionProfile, CodingAgentConfig, ManagerClientMessage, ManagerHostMessage, ManagerImageAttachment, MainAgentConfig, ProfilePatch, SessionSummaryPatch } from "./protocol.js";
...
import { buildImageAwareCompanionPrompt, resolveManagedImagePath, stageManagedImages, type StagedManagerImage } from "./image-staging.js";
import { CodexObservationController, CodexObservationStore, type CodexObservationProgressEvent, type CodexObservationRequest } from "./codex-observation.js";
import { probeCodexAppServer } from "./codex-worker.js";
import { VisionWebDetectionClient, type VisionWebDetectionResult } from "./vision-web-detection.js";
...
async function handle(message: ManagerClientMessage, send: Send, socket?: net.Socket): Promise<void> {
  ...
  if (message.type === "delete_manager_session") {
    ...
    try { await new CodexObservationStore().deleteSession(message.sessionId); }
    catch (error) { cleanupFailures.push(`删除 Codex 观察数据失败：${error instanceof Error ? error.message : "未知错误"}`); }
...
class ManagerSession {
  ...
  private readonly codexObservation: CodexObservationController;
  private readonly visionWebDetection = new VisionWebDetectionClient();
  private visionRequestActive = false;
  ...
  private readonly activeImageAttachments = new Map<string, StagedManagerImage>();
  ...
  private readonly codexTaskLeases = new Map<string, WorkspaceLease>();
  ...
  private emitCodexProgress(event: CodexObservationProgressEvent): void {
    switch (event.type) {
  ...
  async prompt(userMessage: string, attachments: ManagerImageAttachment[] = [], executor?: Extract<ManagerClientMessage, { type: "send_manager_message" }>['executor'], draftId?: string, generateTitle = false): Promise<void> {
    ...
    if (codexPrompt !== undefined || hasCodexPrefix || executor === "codex") {
      try {
        ...
        const code = error instanceof ManagerError || typeof (error as any)?.code === "string" ? (error as any).code : "CODEX_ERROR";
    ...
    const codexState = codexStatus.ok ? String((codexStatus.data as any)?.state ?? "") : "";
    ...
    try {
      ...
      for (const image of staged) this.activeImageAttachments.set(image.attachmentId, image);
  ...
  cancel(target: "antigravity" | "codex" | "all" = "all"): void {
    ...
    if (target === "codex" || target === "all") this.codexObservation.cancel();
  }
  ...
  async disposeForDeletion(): Promise<void> {
    ...
    finally { await this.codexObservation.deleteLocalData(); }
  }
  ...
  async handleCodexObservationRequest(request: CodexObservationRequest): Promise<{ ok: boolean; data?: unknown; error?: { code: string; message: string } }> {
    return this.codexObservation.handle(request);
  }

  async handleAgentToolRequest(request: AgentToolRequest): Promise<{ ok: boolean; data?: unknown; error?: { code: string; message: string } }> {
    if (request.sessionId !== this.id) return { ok: false, error: { code: "SESSION_MISMATCH", message: "Agent Tool 请求不属于当前 Manager 会话。" } };
    if (request.operation !== "identify_image") return { ok: false, error: { code: "UNSUPPORTED_OPERATION", message: "不支持的 Agent Tool 操作。" } };
    const attachmentId = request.attachmentId?.trim();
    if (!attachmentId) return { ok: false, error: { code: "ATTACHMENT_REQUIRED", message: "identify_image 需要 attachment_id。" } };
    const image = this.activeImageAttachments.get(attachmentId);
    if (!image || !this.busy) return { ok: false, error: { code: "ATTACHMENT_NOT_AVAILABLE", message: "图片附件不属于当前正在处理的 Antigravity 回合。" } };
    if (this.visionRequestActive) return { ok: false, error: { code: "VISION_BUSY", message: "当前回合已经有一个网页识图请求在执行。" } };

    const managedPath = await resolveManagedImagePath(this.runtime!, this.id, image).catch(() => undefined);
    if (!managedPath) return { ok: false, error: { code: "ATTACHMENT_INVALID", message: "图片附件在识图前未通过受管路径校验。" } };
  ...
  updateCodexApprovalPolicy(policy?: string): void {
    this.codexObservation.setApprovalPolicy(policy);
  }

  updateCodexSandboxMode(mode?: string): void {
    this.codexObservation.setSandboxMode(mode);
  }

  updateCodexConfiguration(config?: Extract<CodingAgentConfig, { provider: "codex" }>): void {
  ...
  async listAgentModels(provider: "antigravity" | "codex"): Promise<{ available: boolean; authenticated: boolean; models: Array<{ id: string; displayName: string; efforts: string[] }>; message?: string }> {
  ...
  async resolveCodingInteraction(requestId: string, approved: boolean, values?: Record<string, unknown>): Promise<void> {
...
function codexConfigFromStartMessage(message: Extract<ManagerClientMessage, { type: "start_manager_session" }>): Extract<CodingAgentConfig, { provider: "codex" }> | undefined {
  if (message.codingAgent?.provider === "codex") return message.codingAgent;
  const profile = message.executorProfiles?.codex;
  return profile?.provider === "codex" ? profile : undefined;
}

function normalizeInteractionKind(value: string): "command_approval" | "file_approval" | "permissions" | "question" | "mcp_form" | "mcp_url" {
...
function isWorkspaceMutationRequest(userMessage: string, attachments: readonly ManagerImageAttachment[]): boolean {
  ...
  if (/^(@?codex)\b/i.test(normalized)) return true;
...
function visionStatusText(result: VisionWebDetectionResult): string {
