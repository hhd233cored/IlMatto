...
test("Codex native approval policies and the always extension behave correctly", async () => {
  ...
  child.stdin = new Writable({
    write(chunk: Buffer, _encoding, callback) {
      ...
      queueMicrotask(() => {
        ...
        if (message.method === "account/read") child.stdout.emit("data", `${JSON.stringify({ id: message.id, result: { account: { type: "chatgpt" } } })}\n`);
        if (message.method === "configRequirements/read") child.stdout.emit("data", `${JSON.stringify({ id: message.id, result: {} })}\n`);
        if (message.method === "thread/start") child.stdout.emit("data", `${JSON.stringify({ id: message.id, result: { thread: { id: "thread-approval" } } })}\n`);
      });
      callback();
    },
  });
  ...
  const events: any[] = [];
  const bridge = new CodexAppServerBridge({ sessionId: "approval", workspacePath: ".", mode: "observation", approvalPolicy: "always", sandboxMode: "read-only" }, (event) => events.push(event), (() => child) as any);
  await bridge.start();
  ...
  (bridge as any).turnAutoApproveRequests = true;

  (bridge as any).onServerRequest({ id: 41, method: "item/commandExecution/requestApproval", params: { command: "npm test" } });
  ...
  (bridge as any).onServerRequest({ id: 411, method: "mcpServer/elicitation/request", params: { message: "填写项目名称" } });
  ...
  (bridge as any).turnAutoApproveRequests = false;
  (bridge as any).onServerRequest({ id: 42, method: "item/new/requestApproval", params: { reason: "未知审批类型", command: "npm test" } });
...
test("Codex coding turns append local image inputs after the original text request", () => {
  ...
  assert.match((input[0] as { type: "text"; text: string }).text, /根据截图修复布局/);
...
test("Codex observation turns use plain text without output schemas", async () => {
  ...
  child.stdin = new Writable({
    write(chunk: Buffer, _encoding, callback) {
      ...
      queueMicrotask(() => {
        ...
        if (message.method === "account/read") child.stdout.emit("data", `${JSON.stringify({ id: message.id, result: { account: { type: "chatgpt" } } })}\n`);
        if (message.method === "configRequirements/read") child.stdout.emit("data", `${JSON.stringify({ id: message.id, result: {} })}\n`);
        if (message.method === "thread/start") child.stdout.emit("data", `${JSON.stringify({ id: message.id, result: { thread: { id: "thread-1" } } })}\n`);
  ...
  child.kill = () => { queueMicrotask(() => { child.killed = true; child.emit("exit", 0, null); }); return true; };
  const events: any[] = [];
  const bridge = new CodexAppServerBridge({ sessionId: "s", workspacePath: ".", mode: "observation", approvalPolicy: "never" }, (event) => events.push(event), (() => child) as any);
  await bridge.start();
