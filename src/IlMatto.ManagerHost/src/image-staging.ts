...
export type StagedManagerImage = {
  attachmentId: string;
  sourcePath: string;
  runtimePath: string;
};
 ...
 */
export async function stageManagedImages(
  runtime: ManagerRuntime,
  sessionId: string,
  attachments: readonly ManagerImageAttachment[],
): Promise<StagedManagerImage[]> {
  ...
  if (!managedRootReal) throw new ImageStagingError("IMAGE_ATTACHMENT_MISSING", "当前会话的受管图片目录不存在，请重新选择或粘贴图片。");
...
export function buildImageAwareTurnPrompt(userMessage: string, images: readonly StagedManagerImage[]): string {
  const paths = images.map((image) => `- attachment_id: ${image.attachmentId}\n  managed_path: "${image.runtimePath}"`).join("\n");
