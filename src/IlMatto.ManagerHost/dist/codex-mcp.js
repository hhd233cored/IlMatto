...
class ManagerRpcClient {
    ...
    requestAgentTool(operation, args) {
        const requestId = `mcp-${randomUUID()}`;
        const message = {
    ...
    onData(chunk) {
        ...
        while (newline >= 0) {
            ...
            if ((message.type !== "codex_observation_response" && message.type !== "agent_tool_response" && message.type !== "companion_memory_response") || typeof message.requestId !== "string")
                continue;
...
async function callTool(name, args) {
    ...
    if (name === "identify_image")
        return client.requestAgentTool("identify_image", args);
