...
export class CompanionMemoryStore {
    ...
    get profilePath() { return path.join(this.root, "profile.md"); }
    ...
    async readSummary(sessionId, title = sessionId) {
    ...
    async updateSummary(sessionId, patch, title = sessionId) {
        const safeId = safeSessionId(sessionId);
    ...
    async ensureTranscript(sessionId, history) {
        const safeId = safeSessionId(sessionId);
    ...
    async searchSessions(query, limit = MAX_SEARCH_RESULTS) {
        await this.initialize();
    ...
    async openSession(sessionId, query) {
        ...
        for (const hit of hits) {
            ...
            if (indexes.some((index) => selected.has(index)))
                continue;
...
function emptySummary(sessionId, title) {
...
function normalizeSummary(value, sessionId, title) {
...
function normalizeList(value, limit) {
    return Array.isArray(value) ? mergeList([], value.filter((item) => typeof item === "string"), limit) : [];
}
function mergeList(current, additions, limit) {
    const result = [...current];
    for (const item of additions ?? []) {
        const normalized = item.trim();
        if (!normalized || result.some((existing) => normalizeForCompare(existing) === normalizeForCompare(normalized)))
            continue;
        result.push(clip(normalized, 500));
    }
    return result.slice(-limit);
}
function joinSummary(current, patch) {
    const addition = patch?.trim();
    if (!addition)
        return current.trim();
    if (!current.trim())
        return addition;
    if (normalizeForCompare(current).includes(normalizeForCompare(addition)))
        return current.trim();
...
function mergeProfile(existing, patch) {
    ...
    for (const item of patch.add ?? []) {
        ...
        if (!kept.some((line) => normalizeForCompare(line.replace(/^[-*]\s+/, "").trim()) === comparable))
            kept.push(`- ${clip(normalized, 500)}`);
    }
    lines.splice(start + 1, end - start - 1, ...kept);
    return lines.join("\n").replace(/\n{3,}/g, "\n\n").trim();
}
function normalizeForCompare(value) { return value.toLowerCase().replace(/\s+/g, " ").trim(); }
function normalizeSearchText(value) { return value.toLowerCase().replace(/\s+/g, " ").trim(); }
function searchTerms(value) { return value.match(/[a-z0-9_]+|[\u4e00-\u9fff]+/gi) ?? [value]; }
function scoreSearch(query, terms, haystack, date) {
    if (!haystack)
        return 0;
    let score = haystack.includes(query) ? 10 : 0;
    for (const term of terms)
        if (term && haystack.includes(term))
            score += Math.max(1, Math.min(4, term.length / 2));
    const timestamp = Date.parse(date);
    if (score > 0 && Number.isFinite(timestamp))
        score += Math.max(0, Math.min(1, (timestamp - Date.now() + 31_536_000_000) / 31_536_000_000));
    return score;
}
...
function fitSnippetMessages(messages, budget) {
...
async function withWriteLock(filePath, operation) {
...
//# sourceMappingURL=companion-memory.js.map