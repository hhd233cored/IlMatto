...
export class CodexObservationStore {
    ...
    async createDraft(sessionId, workspacePath, prompt, attachments) {
    ...
    async readDraft(draftId) {
        try {
            const draft = JSON.parse(await readFile(this.draftPath(draftId), "utf8"));
    ...
    async saveReport(report, diff = "") {
        await mkdir(this.root, { recursive: true });
        await writeFile(this.reportPath(report.taskId), JSON.stringify(report, null, 2), "utf8");
        if (diff)
            await writeFile(this.diffPath(report.taskId), diff, "utf8");
    ...
    async getReport(sessionId, taskId) {
        const index = await this.readIndex();
        const entry = index[sessionId];
        const selected = taskId ?? entry?.latestTaskId;
        if (!selected || (taskId && !(entry?.taskIds ?? []).includes(taskId)))
            return undefined;
        try {
            const report = JSON.parse(await readFile(this.reportPath(selected), "utf8"));
    ...
    async getDiff(sessionId, taskId, maxBytes = 256_000) {
    ...
    async setThreadId(sessionId, threadId) {
    ...
    async updateIndex(sessionId, update) {
        const previous = this.indexQueue;
        const operation = previous.then(async () => {
            await mkdir(this.root, { recursive: true });
            const index = await this.readIndexFile();
    ...
    draftPath(id) { return path.join(this.root, `${id}.json`); }
    eventPath(id) { return path.join(this.root, `${id}.events.jsonl`); }
    reportPath(id) { return path.join(this.root, `${id}.report.json`); }
    diffPath(id) { return path.join(this.root, `${id}.diff.txt`); }
}
export class CodexObservationController {
    ...
    async handle(request) {
        if (request.sessionId !== this.sessionId)
            return fail("SESSION_MISMATCH", "Codex 观察请求不属于当前 Manager 会话。");
        try {
            switch (request.operation) {
                ...
                case "get_codex_report": {
                    if (!request.taskId)
                        return fail("INVALID_REQUEST", "缺少 taskId。");
    ...
    async resolveInteraction(requestId, approved, values) {
        ...
        if (draft) {
            ...
            if (hashPrompt(draft.prompt) !== draft.promptHash)
                throw new Error("Codex 草稿校验失败：Prompt 已被修改或损坏。");
            ...
            if (path.resolve(draft.workspacePath).toLowerCase() !== this.workspacePath.toLowerCase())
                throw new Error("Codex 草稿工作区已发生变化。");
            this.drafts.delete(requestId);
            await this.store.deleteDraft(requestId);
        ...
        if (!this.bridge)
            throw new Error("Codex 交互请求不存在或已完成。");
     ...
     * validated here. */
    async submitPrompt(prompt, attachments = [], draftId) {
        ...
        if (draftId) {
            ...
            if (!draft)
                throw new Error("Codex 草稿不存在或已过期。");
            if (draft.sessionId !== this.sessionId)
                throw new Error("Codex 草稿不属于当前 Manager 会话。");
            if (path.resolve(draft.workspacePath).toLowerCase() !== this.workspacePath.toLowerCase())
                throw new Error("Codex 草稿工作区已发生变化。");
    ...
    async createDraft(request) {
        if (this.activeTask && !this.activeTask.finished)
            return Promise.reject(new Error("Codex 正在处理另一个任务。"));
        const prompt = request.prompt?.trim() ?? "";
        if (!prompt)
            return Promise.reject(new Error("Codex Prompt 不能为空。"));
        const workspacePath = path.resolve(request.workspacePath ?? this.workspacePath);
        if (workspacePath.toLowerCase() !== this.workspacePath.toLowerCase())
            return Promise.reject(new Error("Codex 任务工作区必须与当前 Manager 工作区一致。"));
    ...
    async startTask(prompt, attachments, statusMessage) {
        if (this.activeTask && !this.activeTask.finished)
            throw new Error("Codex 已有任务正在运行。");
    ...
    async ensureBridge() {
        ...
        try {
            ...
            if (!(storedThreadId && error instanceof CodexWorkerError && error.code === "CODEX_THREAD_RESUME_FAILED"))
                throw error;
            bridge = this.newBridge(undefined);
            await bridge.start();
        }
        this.bridge = bridge;
        if (bridge.sessionRef)
            await this.store.setThreadId(this.sessionId, bridge.sessionRef);
        return bridge;
    }
    newBridge(threadId) {
        const settings = {
    ...
    async onBridgeEvent(event) {
        ...
        if (type === "thinking_delta") {
            const text = redactObservationDelta(event.text ?? "", 12_000);
            if (text)
                this.onEvent?.({ type: "thinking_delta", taskId: active.taskId, text });
            return;
        }
        if (type === "tool_output") {
            const text = redactObservationDelta(event.text ?? event.output ?? "", 12_000);
            if (text)
                this.onEvent?.({ type: "tool_output", taskId: active.taskId, callId: String(event.callId ?? ""), tool: String(event.tool ?? "工具"), text });
            return;
        }
        ...
        if (type === "interaction_request") {
            ...
            if (details && !active.pendingQuestions.includes(details))
                active.pendingQuestions.push(redactObservationText(details, 600));
        ...
        if (type === "error") {
            ...
            if (message && !active.warnings.includes(message))
                active.warnings.push(message);
            return;
        }
    ...
    collectToolResult(active, event) {
        ...
        if (tool === "run_command") {
            ...
            if (isTestCommand(command))
                active.tests.push({ name: command, status: event.ok === false ? "failed" : exitCode !== undefined && exitCode !== 0 ? "failed" : "passed", summary });
            if (event.ok === false || (exitCode !== undefined && exitCode !== 0))
                active.warnings.push(summary || `命令失败：${command}`);
        }
        if (tool === "modify_files") {
            for (const file of command.split(/\r?\n/).map((item) => item.trim()).filter(Boolean)) {
                if (!active.changedFiles.some((entry) => entry.path === file))
                    active.changedFiles.push({ path: file });
            }
            if (typeof event.diff === "string" && event.diff)
                active.diff = `${active.diff}${active.diff ? "\n" : ""}${event.diff}`.slice(-512_000);
        }
        if (summary && event.ok === false && !active.warnings.includes(summary))
            active.warnings.push(summary);
    }
    async finishTask(active, state, text, _sourceStatus) {
        if (active.finished && this.activeTask !== active)
            return;
        active.finished = true;
...
function defaultObservationRoot() {
...
function mapObservationStatus(value) {
    ...
    if (status.includes("cancel") || status.includes("interrupt"))
        return "cancelled";
...
function parseExitCode(summary) {
...
function redactObservationText(value, maximum) {
 ...
 */
function redactObservationDelta(value, maximum) {
    const text = String(value ?? "")
        .replace(/(sk-[A-Za-z0-9_-]{8,}|AIza[\w-]{20,}|Bearer\s+[A-Za-z0-9._~+\/-]{12,})/gi, "[已脱敏]")
        .replace(/((?:api|access|auth|refresh)[_-]?token|password|secret)\s*[=:]\s*[^\s"']+/gi, "$1=[已脱敏]");
    return text.length <= maximum ? text : `${text.slice(0, Math.max(0, maximum - 1))}…`;
}
function appendPrefix(current, chunk, maximum) {
    if (!chunk || current.length >= maximum)
        return current;
 ...
 */
function completionSuffix(full, streamed) {
    ...
    while (streamedIndex < streamedText.length) {
        while (fullIndex < finalText.length && /\s/.test(finalText[fullIndex]))
            fullIndex++;
        while (streamedIndex < streamedText.length && /\s/.test(streamedText[streamedIndex]))
            streamedIndex++;
        if (streamedIndex >= streamedText.length)
            break;
        if (fullIndex >= finalText.length || finalText[fullIndex] !== streamedText[streamedIndex]) {
            ...
            if (compactFinal && compactStreamed.startsWith(compactFinal))
                return "";
            return finalText;
        }
        fullIndex++;
...
function redactObservationDetails(event) {
    const result = {};
    for (const [key, value] of Object.entries(event)) {
        if (["type", "timestamp"].includes(key))
            continue;
        if (typeof value === "string")
            result[key] = redactObservationText(value, key === "output" || key === "diff" ? 12_000 : 3_000);
...
//# sourceMappingURL=codex-observation.js.map