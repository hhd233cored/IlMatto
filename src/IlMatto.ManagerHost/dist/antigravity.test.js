...
test("Antigravity exposes each new tool call as a distinct stream event", async () => {
    ...
    const session = new AntigravitySession("agy", testRuntime(), 2, "medium", undefined, undefined, ((_file, _args) => process));
    try {
        const events = [];
...
test("a complete streamed action followed by a clean exit is still a successful turn", async () => {
    const launches = [];
    const runtime = { root: "C:\\runtime\\ilmatto", schemaPath: "C:\\runtime\\schema.json", logPath: "C:\\runtime\\agy.log", agentName: "ilmatto-manager-test" };
...
test("one live stream-json process handles consecutive turns without restarting", async () => {
    ...
    const session = new AntigravitySession("agy", runtime, 2, "medium", undefined, undefined, ((_file, args) => {
        launches.push(args);
        return process;
    }));
    try {
...
test("a failed AGY conversation resume restarts once without injecting persisted history", async () => {
    ...
    const launches = [];
    const session = new AntigravitySession("agy", runtime, 2, "medium", undefined, "saved-conversation", ((_file, args) => { launches.push(args); return processes.shift(); }), "restart-resume-test", false, [{ role: "user", text: "之前我们讨论过会话恢复" }, { role: "assistant", text: "这是历史上下文" }]);
    try {
...
test("a successful AGY conversation resume is recorded in lifecycle diagnostics", async () => {
    ...
    const session = new AntigravitySession("agy", runtime, 2, "medium", undefined, "persisted-conversation", (() => process), "restart-resume-success-test", false, [{ role: "user", text: "历史消息" }]);
    try {
...
test("unified text mode uses one full-permission AGY process without schema flags", async () => {
    ...
    try {
        const events = [];
        const turn = await session.ask("请检查并测试项目", (event) => { if (event.kind === "text")
...
test("unified text mode accepts nested AGY response text", async () => {
    ...
    const session = new AntigravitySession("agy", testRuntime(), 2, "medium", undefined, undefined, (() => process), "unified-nested", false);
    try {
...
test("Antigravity bootstrap is sent once per process and re-sent after a restart", async () => {
    ...
    const session = new AntigravitySession("agy", testRuntime(), 2, "medium", undefined, undefined, (() => processes.shift()), "bootstrap-test", false);
    try {
...
test("a stale process exit cannot reject a new process turn", async () => {
    ...
    const session = new AntigravitySession("agy", testRuntime(), 2, "medium", undefined, undefined, (() => processes.shift()));
...
test("an incomplete streamed response exits as AGY_EARLY_EXIT without a hidden repair process", async () => {
    ...
    try {
        await assert.rejects(session.ask("image question"), (error) => error instanceof AntigravitySessionError && error.code === "AGY_EARLY_EXIT");
        assert.equal(launches.length, 1);
    }
    finally {
        session.dispose();
...
function testRuntime() {
    return { root: "C:\\runtime\\ilmatto", schemaPath: "C:\\runtime\\schema.json", logPath: "C:\\runtime\\agy.log", agentName: "ilmatto-manager-test" };
}
...
class ScriptedAntigravityProcess extends EventEmitter {
    ...
    stdout = Object.assign(new EventEmitter(), { setEncoding: (_encoding) => undefined });
    stderr = Object.assign(new EventEmitter(), { setEncoding: (_encoding) => undefined });
    constructor(script) {
        super();
        this.script = script;
    }
    emitInit(conversationId = "conversation-live") {
...
//# sourceMappingURL=antigravity.test.js.map