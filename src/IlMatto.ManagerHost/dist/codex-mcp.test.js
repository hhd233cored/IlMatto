...
test("IlMatto Agent Tools MCP exposes Codex, image lookup, and companion memory tools", async () => {
    ...
    try {
        ...
        const identifyImage = tools.find((tool) => tool.name === "identify_image");
        ...
        const sessionOpen = tools.find((tool) => tool.name === "session_open");
        assert.deepEqual(sessionOpen.inputSchema.required, ["session_id", "query"]);
        const sessionUpdate = tools.find((tool) => tool.name === "session_update");
        assert.deepEqual(sessionUpdate.inputSchema.required, ["patch"]);
        const profileUpdate = tools.find((tool) => tool.name === "profile_update");
