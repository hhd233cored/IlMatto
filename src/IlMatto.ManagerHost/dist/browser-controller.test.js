...
test("BrowserController reports a structured error when Chrome is unavailable", async () => {
    ...
    const controller = new BrowserController((event) => events.push(event));
    try {
        await assert.rejects(controller.handle("session-a", { type: "browser_request", sessionId: "session-a", requestId: "r1", operation: "start" }), (error) => error?.code === "BROWSER_CHROME_NOT_FOUND");
        ...
        if (previous === undefined)
            delete process.env.ILMATTO_BROWSER_CHROME_PATH;
        else
            process.env.ILMATTO_BROWSER_CHROME_PATH = previous;
        if (previousProfile === undefined)
            delete process.env.ILMATTO_BROWSER_PROFILE;
        else
            process.env.ILMATTO_BROWSER_PROFILE = previousProfile;
...
test("BrowserController detects human verification and locks writes", async () => {
    const events = [];
    const controller = new BrowserController((event) => events.push(event));
    controller.ownerSessionId = "session-a";
    controller.state = "AgentControlled";
    ...
    await assert.rejects(controller.handle("session-a", { type: "browser_request", sessionId: "session-a", requestId: "r3", operation: "click", ref: "e1" }), (error) => error?.code === "BROWSER_CONTROLLED_BY_HUMAN");
...
test("BrowserController keeps the headed browser visible after human verification", async () => {
    ...
    controller.ownerSessionId = "session-a";
    controller.state = "WaitingForHuman";
...
test("BrowserController retries startup when the desktop opens a browser in Error state", async () => {
    ...
    controller.start = async () => {
        starts += 1;
        controller.state = "AgentControlled";
        return controller.snapshotState("session-a");
    };
...
//# sourceMappingURL=browser-controller.test.js.map