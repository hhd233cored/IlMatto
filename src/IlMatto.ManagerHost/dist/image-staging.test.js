...
test("managed image staging rejects a path outside the current session attachment directory", async () => {
    ...
    try {
        ...
        await assert.rejects(stageManagedImages(runtime, sessionId, [{ type: "image", path: outside }]), /受管附件目录/);
        const stagedFiles = await readdir(path.join(runtime.attachmentsRoot, sessionId)).catch(() => []);
...
//# sourceMappingURL=image-staging.test.js.map