...
class BrowserRpcClient {
    ...
    request(operation, args = {}) {
        if (!this.connected && this.socket.readyState !== "open")
            return Promise.reject(new Error("ManagerHost 尚未连接。"));
        ...
        const message = {
            ...
            page: isRecord(args.page) ? args.page : undefined,
        };
    ...
    onData(chunk) {
        ...
        while (newline >= 0) {
            ...
            if (message.type !== "browser_action_result" || typeof message.requestId !== "string")
                continue;
...
class BrowserMcpSession {
    ...
    async ensureBrowser() {
        ...
        if (typeof endpoint !== "string" || !endpoint)
            throw new BrowserMcpError("BROWSER_CDP_CONNECT_FAILED", "BrowserHost 未返回 CDP 地址。");
        if (this.browser?.isConnected() && this.cdpEndpoint === endpoint)
            return this.browser;
        try {
            this.detachBrowserConnection();
    ...
    detachBrowserConnection() {
        ...
        try {
            connection?.close?.("Browser MCP detached");
        }
    ...
    async tabs() {
        ...
        for (const context of browser.contexts()) {
            for (const page of context.pages())
                tabs.push({ index: tabs.length, url: page.url(), title: await page.title().catch(() => "") });
        }
        return result(tabs);
    }
    async navigate(args) {
        ...
        if (!/^https?:\/\//i.test(url))
            throw new BrowserMcpError("BROWSER_INVALID_URL", "browser_navigate 只允许 http/https URL。");
    ...
    async snapshot() {
        ...
        if (body)
            lines.push("", body.slice(0, 12_000));
    ...
    async scroll(args) {
        ...
        if (direction === "up")
            ...
            await page.mouse.wheel(direction === "left" ? -amount : amount, 0);
    ...
    async upload(args) {
        ...
        if (!filePath)
            throw new BrowserMcpError("BROWSER_FILE_REQUIRED", "browser_upload 需要文件路径。");
        const absolutePath = path.resolve(filePath);
        const file = await stat(absolutePath).catch(() => undefined);
        if (!file?.isFile())
            throw new BrowserMcpError("BROWSER_FILE_NOT_FOUND", `找不到要上传的文件：${absolutePath}`);
    ...
    async download(args) {
        ...
        if (failure)
            throw new BrowserMcpError("BROWSER_DOWNLOAD_FAILED", failure);
    ...
    async evaluate(args) {
        ...
        if (!expression)
            throw new BrowserMcpError("BROWSER_EXPRESSION_REQUIRED", "browser_evaluate 需要 JavaScript 表达式。");
        if (expression.length > 20_000)
            throw new BrowserMcpError("BROWSER_EXPRESSION_TOO_LARGE", "browser_evaluate 表达式不能超过 20000 个字符。");
    ...
    resolveRef(args) {
        ...
        if (!/^e\d+$/.test(ref) || !this.refs.has(ref))
            throw new BrowserMcpError("BROWSER_INVALID_REF", "无效或已过期的元素 ref，请先调用 browser_snapshot。");
        return this.refs.get(ref);
    }
    async authorize(permission, details, write = true) {
    ...
    async afterPageChange(page) {
        await page.waitForLoadState("domcontentloaded", { timeout: 5_000 }).catch(() => undefined);
        const title = await page.title().catch(() => "");
        await this.reportPageState(page);
        return result({ url: page.url(), title });
    }
    async reportPageState(page, textHint) {
...
function argument(name) { const index = process.argv.indexOf(name); return index >= 0 ? process.argv[index + 1] : process.argv.find((value) => value.startsWith(`${name}=`))?.slice(name.length + 1); }
function isRecord(value) { return Boolean(value) && typeof value === "object" && !Array.isArray(value); }
function isDirection(value) { return value === "up" || value === "down" || value === "left" || value === "right"; }
function isPermission(value) { return value === "navigate" || value === "click" || value === "fill" || value === "press" || value === "scroll" || value === "screenshot" || value === "upload" || value === "download" || value === "evaluate" || value === "coordinate"; }
function isMouseButton(value) { return value === "left" || value === "right" || value === "middle"; }
function finiteCoordinate(value) { const number = typeof value === "number" ? value : Number(value); if (!Number.isFinite(number) || number < 0 || number > 100_000)
    throw new BrowserMcpError("BROWSER_INVALID_COORDINATE", "坐标必须是 0 到 100000 之间的有限数字。"); return number; }
function safeFilename(value) { const filename = value.replace(/[<>:"/\\|?*\u0000-\u001F]/g, "_").trim(); return filename || "download"; }
function jsonSafe(value) { try {
    return JSON.parse(JSON.stringify(value, (_key, item) => typeof item === "bigint" ? `${item}n` : item));
}
catch {
    return String(value);
} }
function errorCode(error) { return typeof error?.code === "string" ? error.code : "BROWSER_MCP_ERROR"; }
