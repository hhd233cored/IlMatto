...
test("unified manager runtime uses the selected workspace without generating an AGY agent", async () => {
    ...
    try {
        ...
        if (previousRuntime === undefined)
            delete process.env.ILMATTO_MANAGER_RUNTIME;
        else
            process.env.ILMATTO_MANAGER_RUNTIME = previousRuntime;
        if (previousLog === undefined)
            delete process.env.ILMATTO_MANAGER_LOG_DIR;
        else
            process.env.ILMATTO_MANAGER_LOG_DIR = previousLog;
        if (previousAgentDir === undefined)
            delete process.env.ILMATTO_MANAGER_AGENT_DIR;
        else
            process.env.ILMATTO_MANAGER_AGENT_DIR = previousAgentDir;
...
test("session cleanup removes owned memory and attachment copies but not the shared profile", async () => {
    ...
    try {
        ...
        if (previousMemory === undefined)
            delete process.env.ILMATTO_COMPANION_MEMORY_DIR;
        else
            process.env.ILMATTO_COMPANION_MEMORY_DIR = previousMemory;
        await rm(workspace, { recursive: true, force: true });
        await rm(runtimeRoot, { recursive: true, force: true });
...
test("manager runtime exposes no executable tools", async () => {
    ...
    try {
        ...
        if (previous === undefined)
            ...
            process.env.ILMATTO_MANAGER_RUNTIME = previous;
        if (previousLog === undefined)
            delete process.env.ILMATTO_MANAGER_LOG_DIR;
        else
            process.env.ILMATTO_MANAGER_LOG_DIR = previousLog;
        ...
        if (previousSettings === undefined)
            delete process.env.ILMATTO_MANAGER_SETTINGS_PATH;
        else
            process.env.ILMATTO_MANAGER_SETTINGS_PATH = previousSettings;
