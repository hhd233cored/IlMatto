...
let mcpConfigQueue = Promise.resolve();
async function withMcpConfigLock(operation) {
    const previous = mcpConfigQueue;
    let release;
    mcpConfigQueue = new Promise((resolve) => { release = resolve; });
    await previous;
    try {
...
function isGeneratedWorkspaceMcpName(name) {
...
export const companionAgentTools = ["view_file", "search_web", "read_url_content", "invoke_subagent"];
 ...
 */
export function buildManagedImageReadPermissionRules(attachmentsRoot) {
...
export function buildCompanionWebPermissionRules() {
    return ["read_url(*)", "execute_url(*)"];
}
export function buildManagerPermissionRules(attachmentsRoot) {
    return [...buildManagedImageReadPermissionRules(attachmentsRoot), ...buildCompanionWebPermissionRules()];
}
async function allowManagedImageReads(settingsPath, attachmentsRoot) {
    let settings = {};
    try {
        ...
        if (error?.code !== "ENOENT")
            throw new Error(`无法读取 Antigravity settings.json：${error instanceof Error ? error.message : "文件格式无效"}`);
    }
...
function renderManagerAgent(agentName, systemPrompt) {
...
export async function ensureManagerRuntime(profile) {
...
export async function ensureUnifiedManagerRuntime(workspacePath, _profile, options) {
    const localAppData = process.env.LOCALAPPDATA ?? path.join(os.homedir(), "AppData", "Local");
    const memoryRoot = path.resolve(process.env.ILMATTO_COMPANION_MEMORY_DIR ?? path.join(localAppData, "IlMatto", "companion-memory"));
    ...
    let browserMcpMount;
    let mcpMountError;
    let browserMcpMountError;
    const mcpOptions = options?.mcp;
    const browserOptions = options?.browser;
    const allMcpOptions = [mcpOptions, browserOptions].filter((item) => Boolean(item));
...
async function cleanupLegacyWorkspaceMcp(workspacePath, options) {
    ...
    try {
        ...
        for (const [name, value] of Object.entries(remaining)) {
            if (!isGeneratedWorkspaceMcpName(name) || !value || typeof value !== "object" || Array.isArray(value))
                continue;
            const entry = value;
        ...
        if (Object.keys(remaining).length === 0 && Object.keys(config).every((key) => key === "mcpServers"))
            ...
            await writeFile(configPath, JSON.stringify({ ...config, mcpServers: remaining }, null, 2), "utf8");
    }
    catch {
...
async function cleanupLegacyWorkspaceMcpPlugins(workspacePath, options) {
    const pluginsRoot = path.join(workspacePath, ".agents", "plugins");
    const registryPath = path.join(workspacePath, ".agents", "plugins.json");
    let pluginNames;
    try {
        pluginNames = (await readdir(pluginsRoot, { withFileTypes: true })).filter((entry) => entry.isDirectory() && isGeneratedWorkspaceMcpName(entry.name)).map((entry) => entry.name);
    }
    catch {
        return;
    }
    ...
    for (const pluginName of pluginNames) {
        ...
        try {
            ...
            const servers = config?.mcpServers;
            const values = servers && typeof servers === "object" && !Array.isArray(servers) ? Object.values(servers) : [];
            if (manifest?.name !== pluginName || values.length !== 1 || !options.some((option) => isIlMattoGeneratedDefinition(values[0], option)))
                continue;
    ...
    try {
        ...
        const remaining = parsed.entries.filter((item) => {
            if (!item || typeof item !== "object" || item.path !== relativePath || !Array.isArray(item.include_only) || item.include_only.length !== 1 || typeof item.include_only[0] !== "string")
                return true;
            return !staleNames.some((name) => item.include_only[0] === `^${escapeRegex(name)}$`);
        });
        if (remaining.length === parsed.entries.length)
            return;
        if (remaining.length === 0 && Object.keys(parsed).every((key) => key === "entries"))
            await rm(registryPath, { force: true });
        else
            await writeFile(registryPath, JSON.stringify({ ...parsed, entries: remaining }, null, 2), "utf8");
    }
 ...
 */
async function mountWorkspaceMcp(workspacePath, options, namePrefix = CURRENT_MCP_PREFIX) {
    if (!existsSync(options.scriptPath))
        throw new Error(`找不到 MCP Server：${options.scriptPath}`);
    ...
    const manifest = { name: serverName };
    let existingManifest;
    try {
        const parsed = JSON.parse(await readFile(manifestPath, "utf8"));
        if (!parsed || typeof parsed !== "object" || Array.isArray(parsed))
            throw new Error("IlMatto MCP plugin.json 根节点必须是对象。");
        existingManifest = parsed;
    }
    catch (error) {
        if (error?.code !== "ENOENT")
            throw new Error(`无法读取 IlMatto MCP plugin.json：${error instanceof Error ? error.message : "文件格式无效"}`);
    }
    if (existingManifest && !deepEqual(existingManifest, manifest))
        throw new Error(`IlMatto MCP plugin 名称已被占用：${serverName}`);
    let config = {};
    try {
        const parsed = JSON.parse(await readFile(configPath, "utf8"));
        if (!parsed || typeof parsed !== "object" || Array.isArray(parsed))
            throw new Error("IlMatto MCP mcp_config.json 根节点必须是对象。");
        config = parsed;
    }
    catch (error) {
        if (error?.code !== "ENOENT")
            throw new Error(`无法读取 IlMatto MCP 配置：${error instanceof Error ? error.message : "文件格式无效"}`);
    }
 ...
 */
async function ensureWorkspacePluginRegistration(registryPath, entry, pluginName) {
    let registry = {};
    try {
        const parsed = JSON.parse(await readFile(registryPath, "utf8"));
        if (!parsed || typeof parsed !== "object" || Array.isArray(parsed))
            throw new Error("plugins.json 根节点必须是对象。");
        registry = parsed;
    }
    catch (error) {
        if (error?.code !== "ENOENT")
            throw new Error(`无法读取 Antigravity plugins.json：${error instanceof Error ? error.message : "文件格式无效"}`);
    }
...
function registrationIncludesPlugin(entry, pluginName) {
    ...
    return filters.some((filter) => {
        if (typeof filter !== "string")
            return false;
        try {
            return new RegExp(filter).test(pluginName);
        }
        catch {
            return false;
        }
 ...
 */
async function mountGlobalMcp(options, namePrefix = CURRENT_MCP_PREFIX) {
    return withMcpConfigLock(() => mountGlobalMcpUnlocked(options, namePrefix));
}
async function mountGlobalMcpUnlocked(options, namePrefix = CURRENT_MCP_PREFIX) {
    if (!existsSync(options.scriptPath))
        throw new Error(`找不到 MCP Server：${options.scriptPath}`);
    ...
    try {
        const parsed = JSON.parse(await readFile(configPath, "utf8"));
        if (!parsed || typeof parsed !== "object" || Array.isArray(parsed))
            throw new Error("Antigravity 全局 mcp_config.json 根节点必须是对象。");
        config = parsed;
    }
    catch (error) {
        if (error?.code !== "ENOENT")
            throw new Error(`无法读取 Antigravity 全局 mcp_config.json：${error instanceof Error ? error.message : "文件格式无效"}`);
    }
    const existingServers = config.mcpServers;
    if (existingServers !== undefined && (!existingServers || typeof existingServers !== "object" || Array.isArray(existingServers))) {
    ...
    if (legacyDefinition !== undefined && isIlMattoGeneratedDefinition(legacyDefinition, options))
        delete servers[LEGACY_GLOBAL_MCP_NAME];
    const previous = servers[serverName];
...
function isIlMattoMcpDefinition(value, options) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return false;
    const entry = value;
    const args = entry.args;
    if (entry.command !== path.resolve(options.command) || !Array.isArray(args) || args[0] !== path.resolve(options.scriptPath))
        return false;
...
function isIlMattoGeneratedDefinition(value, options) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return false;
    const entry = value;
    const args = entry.args;
...
async function cleanupLegacyArtifacts(configuredRuntime, fingerprint, logDirectory) {
    const agentName = `ilmatto-manager-${fingerprint}`;
    const globalAgentsRoot = path.resolve(process.env.ILMATTO_MANAGER_AGENT_DIR ?? path.join(os.homedir(), ".gemini", "config", "agents"));
    const targets = [
...
export async function cleanupManagerRuntime(runtime) {
    if (!runtime)
        return;
    if (runtime.mcpMount)
        await cleanupMcp(runtime.mcpMount);
    if (runtime.browserMcpMount)
        await cleanupMcp(runtime.browserMcpMount);
    ...
    if (runtime.agentPath)
        try {
            await rm(runtime.agentPath, { force: true });
        }
        catch { }
    if (runtime.agentName) {
        try {
            await rm(path.join(path.dirname(runtime.agentPath), runtime.agentName), { recursive: true, force: true });
        }
        catch { }
        try {
            await rm(path.join(runtime.root, ".agents", "agents", `${runtime.agentName}.md`), { force: true });
        }
        catch { }
        try {
            await rm(path.join(runtime.root, ".agents", "agents", runtime.agentName), { recursive: true, force: true });
        }
        catch { }
    }
}
 ...
 */
export async function cleanupManagerSessionData(runtime, sessionId) {
    ...
    for (const target of targets) {
        try {
            await removeManagerSessionTargetWithRetry(target);
        }
        catch (error) {
            failures.push(`${target}: ${error instanceof Error ? error.message : "未知错误"}`);
        }
    }
    ...
    if (failures.length > 0)
        throw new Error(`部分 Manager 会话数据未能删除：\n${failures.join("\n")}`);
}
...
async function removeManagerSessionTargetWithRetry(target) {
...
async function cleanupMcp(mount) {
    await withMcpConfigLock(() => cleanupMcpUnlocked(mount));
}
async function cleanupMcpUnlocked(mount) {
    try {
        ...
        if (!servers || typeof servers !== "object" || Array.isArray(servers))
            return;
        const current = servers[mount.serverName];
        if (!deepEqual(current, mount.definition))
            return;
        const remaining = { ...servers };
        ...
        if (mount.scope === "workspace-plugin" && mount.manifestPath && mount.manifest && mount.pluginDirectory) {
            try {
                ...
                if (deepEqual(currentManifest, mount.manifest)) {
                    ...
                    if (entries.length === 0)
                        await rm(mount.pluginDirectory, { recursive: true, force: true });
                }
            }
            catch { }
        }
