 ...
 */
export class AntigravityWorkerBridge {
    settings;
    onEvent;
    ...
    activeTaskId;
    completed = false;
    agentFailureReported = false;
    agentFallbackRetries = 0;
    restarting = false;
    retryTimer;
    logWatchTimer;
    ...
    get sessionRef() { return undefined; }
    async start() {
        if (this.child)
            return;
        ...
        if (this.settings.model?.trim())
            args.push("--model", this.settings.model.trim());
    ...
    sendCodeTask(taskId, userRequest, _attachments) {
        if (this.agentFailureReported)
            throw new Error("Antigravity 编码 agent 未加载，已停止本次任务。");
        if (this.activeTaskId)
            throw new Error("Antigravity 编码执行器正在处理另一项任务。");
        this.activeTaskId = taskId;
        this.completed = false;
        this.startedToolCalls.clear();
        this.completedToolCalls.clear();
        this.taskTimer = setTimeout(() => {
            if (!this.activeTaskId || this.completed)
                return;
    ...
    resolve(_requestId, _approved, _values) {
    ...
    cancel() {
        ...
        if (this.logWatchTimer) {
            clearInterval(this.logWatchTimer);
            this.logWatchTimer = undefined;
        }
    ...
    async dispose() {
        ...
        await cleanupExecutorRuntime(this.runtime);
        this.runtime = undefined;
    }
    /** AGY writes agent-fallback diagnostics to --log-file rather than stderr. */
    startLogWatcher() {
        if (this.logWatchTimer || !this.runtime)
            return;
        this.logWatchTimer = setInterval(() => {
            if (this.logReadInFlight || !this.runtime || this.agentFailureReported)
                return;
            ...
            void readFile(this.runtime.logPath, "utf8").then((text) => {
                ...
                if (/agent\s+["'`].*?["'`]\s+not found[\s\S]*falling back/i.test(fresh))
    ...
    spawnChild() {
        ...
        child.on("error", (error) => {
            if (this.child === child)
                this.fail("AGY_EXECUTOR_NOT_FOUND", error.message);
        });
        child.on("exit", (code) => {
            if (this.child !== child || this.restarting)
                return;
            if (!this.completed && this.activeTaskId)
                this.fail("AGY_EXECUTOR_EXITED", this.stderr || `Antigravity 执行器退出（${code ?? "未知"}）。`);
        });
    ...
    onStdout(chunk) {
        ...
        while (newline >= 0) {
            ...
            if (!line)
                continue;
            try {
                this.onStreamEvent(JSON.parse(line));
            }
            catch { /* ignore non-NDJSON diagnostics */ }
        }
    }
    onStreamEvent(event) {
        ...
        if (eventName === "result") {
            ...
            if (result)
                ...
                this.finish(failedResult(String(envelope?.error ?? "Antigravity 返回了无法识别的结构化结果。")));
            return;
        }
    ...
    onStepUpdate(step) {
        ...
        if (typeof thinking === "string" && thinking)
            this.onEvent({ type: "thinking_delta", sessionId: this.settings.sessionId, taskId, text: thinking });
        const text = step?.text_delta;
        if (typeof text === "string" && text && !text.trimStart().startsWith("{"))
            this.onEvent({ type: "assistant_delta", sessionId: this.settings.sessionId, taskId, text });
        ...
        if (typeof toolName === "string") {
            ...
            if (output)
                this.onEvent({ type: "tool_output", sessionId: this.settings.sessionId, taskId, callId, tool: toolName, text: output });
    ...
    fail(code, message) {
        ...
        if (taskId && !this.completed)
            this.finish(failedResult(message));
    }
    reportAgentFailure(message, killProcess) {
        if (this.agentFailureReported)
            return;
    ...
    handleAgentFallback(message = "Antigravity 未加载 IlMatto 的编码 agent，而是回退到了默认 agent。请检查生成的 agent 定义和执行器日志。") {
        if (this.agentFailureReported || this.restarting)
            return;
        const config = this.spawnConfig;
        ...
        this.retryTimer = setTimeout(() => {
            this.retryTimer = undefined;
            if (this.completed || this.agentFailureReported)
                return;
            this.restarting = false;
            this.stderr = "";
            this.spawnChild();
...
async function ensureExecutorRuntime(settings) {
...
export function renderExecutorAgent(agentName, settings) {
...
async function cleanupExecutorRuntime(runtime) {
    if (!runtime)
        return;
    await Promise.all([
...
function executionPolicyInstruction(policy) {
    switch (policy) {
 ...
 */
export function commandExecutionPolicy(settings) {
...
export function extractAntigravityWorkerStep(event) {
...
export function parseAntigravityWorkerResult(event) {
...
//# sourceMappingURL=antigravity-worker.js.map