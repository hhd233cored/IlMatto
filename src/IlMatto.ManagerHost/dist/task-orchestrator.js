 ...
 */
export function parseExecutorDirective(input) {
    const match = /^\s*@(?<provider>antigravity|pi|codex)(?=\s|$)\s*/i.exec(input);
    if (!match?.groups?.provider)
        return { request: input };
    return { executor: match.groups.provider.toLowerCase(), request: input.slice(match[0].length) };
}
export function defaultExecutorFor(mainProvider) {
...
export class ExecutionLease {
    ...
    transition(taskId, phase) {
        if (!this.holder || this.holder.taskId !== taskId)
            throw new TaskOrchestratorError("LEASE_NOT_HELD", "任务没有持有工作区执行锁。");
        this.holder.phase = phase;
    }
    release(taskId) {
        if (this.holder?.taskId === taskId)
            this.holder = undefined;
    }
    get active() { return this.holder; }
}
export function buildHandoffPacket(taskId, provider, result, workspaceSnapshotId) {
    return {
        taskId,
        provider,
...
export function privateTraceEvent(taskId, provider, kind, text, details) {
    return {
        taskId,
        provider,
        kind,
...
export function redactAndLimit(value, maximum = 64_000) {
