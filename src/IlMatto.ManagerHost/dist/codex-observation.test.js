...
test("Codex reports are session-scoped and diff reads are bounded", async () => {
    ...
    try {
        const store = new CodexObservationStore(root);
        const report = {
            taskId: "task-1", sessionId: "session-1", state: "completed", summary: "已完成", changedFiles: [], commands: [], tests: [], warnings: [], pendingQuestions: [],
        ...
        assert.equal(diff.data.diff.length <= 15, true);
...
test("confirmed Codex drafts create a plain-text task report", async () => {
    ...
    try {
        ...
        const bridge = {
            ...
            sendCodeTask: (taskId) => {
                queueMicrotask(() => {
                    const emit = (event) => bridgeFactoryEvent?.(event);
...
test("Codex progress is projected for the desktop bubble without exposing raw bridge events", async () => {
    ...
    try {
        ...
        const bridge = {
            ...
            start: async () => undefined,
            sendCodeTask: (taskId) => queueMicrotask(() => {
        ...
        const draft = (await controller.handle({ sessionId: "session-progress", requestId: "draft", operation: "draft_codex_task", workspacePath: root, prompt: "运行测试" })).data;
        await controller.resolveInteraction(draft.draftId, true);
        const deadline = Date.now() + 2_000;
        while (Date.now() < deadline && !progress.some((item) => item.type === "completed"))
            await new Promise((resolve) => setTimeout(resolve, 10));
...
test("Codex completion does not replay streamed text, including multiple message items", async () => {
    ...
    try {
        ...
        const draft = (await controller.handle({ sessionId: "session-dedup", requestId: "draft", operation: "draft_codex_task", workspacePath: root, prompt: "测试去重" })).data;
        await controller.resolveInteraction(draft.draftId, true);
        const deadline = Date.now() + 2_000;
        while (Date.now() < deadline && !progress.some((item) => item.type === "completed"))
            await new Promise((resolve) => setTimeout(resolve, 10));
...
test("missing Codex marks the optional channel unavailable without throwing from the session", async () => {
    ...
    try {
        ...
        const draft = (await controller.handle({ sessionId: "session-1", requestId: "draft", operation: "draft_codex_task", workspacePath: root, prompt: "尝试 Codex" })).data;
        await controller.resolveInteraction(draft.draftId, true);
        const deadline = Date.now() + 2_000;
        let status;
        ...
        const report = (await controller.handle({ sessionId: "session-1", requestId: "report", operation: "get_latest_codex_report" })).data;
...
test("explicit Codex submission starts a task without a draft approval interaction", async () => {
    ...
    try {
        ...
        const bridge = {
            ...
            start: async () => undefined,
            sendCodeTask: (taskId, prompt) => queueMicrotask(() => {
...
//# sourceMappingURL=codex-observation.test.js.map