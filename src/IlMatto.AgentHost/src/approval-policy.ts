export function isSafeCommand(command: string): boolean {
  ...
  if (!normalized || /(^|[;|&])\s*(remove-item|del|erase|rd|rmdir|set-content|add-content|copy-item|move-item|rename-item|format|shutdown|stop-process|taskkill)\b/.test(normalized)) return false;
  if (/\b(invoke-webrequest|invoke-restmethod|curl|wget|start-process|set-executionpolicy|reg\s|git\s+(reset|checkout|clean|restore|commit|push|pull|merge|rebase)|npm\s+(install|uninstall|publish)|dotnet\s+(publish|pack))\b/.test(normalized)) return false;
