...
export type GitFileChange = { path: string; status: string; kind: GitChangeKind };
...
export type GitDiffResult = { scope: GitDiffScope; path?: string; content: string; truncated: boolean };
...
export async function getGitOverview(workspacePath: string): Promise<GitOverview> {
  try {
...
export async function initGitRepository(workspacePath: string): Promise<string> {
  const workspace = normalizeWorkspace(workspacePath);
  const realWorkspace = await fs.realpath(workspace);
  const stat = await fs.stat(realWorkspace);
  if (!stat.isDirectory()) throw new Error("The selected workspace must be a directory");

  try {
    const existingRoot = (await gitOutput(realWorkspace, ["rev-parse", "--show-toplevel"])).trim();
    if (existingRoot) throw new Error(`The workspace is already inside a Git repository at ${existingRoot}`);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    const normalized = message.toLowerCase();
    if (!normalized.includes("not a git repository") && !normalized.includes("not a git repo") && !normalized.includes("outside repository")) throw error;
  }
...
export async function createInitialGitCommit(workspacePath: string, repositoryAlreadyInitialized = false): Promise<string> {
  const repository = await ensureRepository(workspacePath);
  ...
  if (stagedFiles.length === 0) return `Git repository is ready at ${repository.repositoryRoot}; there were no files to commit.`;
...
export async function getGitDiff(workspacePath: string, scope: GitDiffScope, relativePath?: string): Promise<GitDiffResult> {
  const repository = await ensureRepository(workspacePath);
...
export async function getGitLog(workspacePath: string, limit = 25): Promise<string> {
  const repository = await ensureRepository(workspacePath);
  return optionalGitOutput(repository.repositoryRoot, ["log", `-${Math.min(Math.max(limit, 1), 100)}`, "--oneline", "--decorate", "--no-ext-diff"]);
}

export async function getGitBranches(workspacePath: string): Promise<string> {
  const repository = await ensureRepository(workspacePath);
  return gitOutput(repository.repositoryRoot, ["branch", "--verbose", "--no-abbrev"]);
}

export async function getGitRemotes(workspacePath: string): Promise<string> {
  const repository = await ensureRepository(workspacePath);
  return gitOutput(repository.repositoryRoot, ["remote", "-v"]);
}

export async function showGitCommit(workspacePath: string, revision: string): Promise<string> {
  const repository = await ensureRepository(workspacePath);
  if (!revision || revision.startsWith("-")) throw new Error("Invalid Git revision");
...
export async function stageGitFiles(workspacePath: string, paths: string[]): Promise<string> {
  const repository = await ensureRepository(workspacePath);
...
export async function unstageGitFiles(workspacePath: string, paths: string[]): Promise<string> {
  const repository = await ensureRepository(workspacePath);
  const pathspecs = await toGitPaths(repository, paths);
...
export async function createGitBranch(workspacePath: string, name: string): Promise<string> {
  const repository = await ensureRepository(workspacePath);
...
export async function switchGitBranch(workspacePath: string, name: string): Promise<string> {
  const repository = await ensureRepository(workspacePath);
...
export async function commitGitChanges(workspacePath: string, message: string): Promise<string> {
  const repository = await ensureRepository(workspacePath);
  const normalizedMessage = message.trim();
  if (!normalizedMessage || normalizedMessage.length > 4096 || normalizedMessage.includes("\0")) throw new Error("Commit message must contain 1 to 4096 characters");
  ...
  if (allStagedPaths.length === 0) throw new Error("There are no staged changes to commit");
...
export async function getStagedSummary(workspacePath: string): Promise<string> {
  const repository = await ensureRepository(workspacePath);
...
export async function ensureRepository(workspacePath: string): Promise<GitRepositoryContext> {
  const workspace = normalizeWorkspace(workspacePath);
  let root: string;
  try {
  ...
  if (workspaceRelative === ".." || workspaceRelative.startsWith(`..${path.sep}`) || path.isAbsolute(workspaceRelative)) throw new Error("Workspace is outside the discovered Git repository");
  return {
...
async function optionalGitOutput(workspace: string, args: string[]): Promise<string> {
  try { return await gitOutput(workspace, args); } catch { return ""; }
}

function scopeArgs(repository: GitRepositoryContext): string[] {
  return repository.workspacePathspec ? ["--", repository.workspacePathspec] : [];
}

function assertRepositoryWideOperation(repository: GitRepositoryContext, operation: string): void {
  if (!repository.isRepositoryRoot) throw new Error(`${operation} can affect files outside the selected workspace. Select the repository root to continue.`);
}

function isInsideWorkspace(repository: GitRepositoryContext, repositoryRelativePath: string): boolean {
...
async function getAheadBehind(workspace: string): Promise<[number, number]> {
...
async function toGitPaths(repository: GitRepositoryContext, paths: string[]): Promise<string[]> {
  if (!Array.isArray(paths) || paths.length === 0) throw new Error("At least one repository-relative path is required");
  return Promise.all(paths.map((relativePath) => toGitPath(repository, relativePath)));
}

async function toGitPath(repository: GitRepositoryContext, relativePath: string): Promise<string> {
  if (!relativePath || path.isAbsolute(relativePath)) throw new Error("Git paths must be relative to the workspace");
  ...
  const relative = path.relative(repository.workspaceRoot, candidate);
  if (!relative || relative === ".") throw new Error("Git paths must name a file or directory inside the workspace");
  return path.relative(repository.repositoryRoot, candidate).split(path.sep).join("/");
}

async function validateBranchName(workspace: string, name: string): Promise<void> {
  const normalized = name.trim();
  if (!normalized || normalized.startsWith("-")) throw new Error("Invalid branch name");
  await gitOutput(workspace, ["check-ref-format", "--branch", normalized]);
}

function parseStatus(value: string, repository: GitRepositoryContext): Pick<GitOverview, "staged" | "unstaged" | "untracked"> {
  ...
  for (const line of value.split(/\r?\n/)) {
    ...
    if (!file) continue;
    if (x === "?" && y === "?") { untracked.push({ path: file, status: "??", kind: "untracked" }); continue; }
    if (x !== " ") staged.push({ path: file, status: x, kind: "staged" });
    if (y !== " ") unstaged.push({ path: file, status: y, kind: "unstaged" });
  }
  return { staged, unstaged, untracked };
}

function parseBranches(value: string): GitBranch[] {
...
function truncateDiff(scope: GitDiffScope, relativePath: string | undefined, content: string): GitDiffResult {
  if (content.length <= MAX_OUTPUT) return { scope, path: relativePath, content, truncated: false };
  return { scope, path: relativePath, content: `${content.slice(0, MAX_OUTPUT)}\n[diff truncated]`, truncated: true };
}

function emptyOverview(message: string): GitOverview {
