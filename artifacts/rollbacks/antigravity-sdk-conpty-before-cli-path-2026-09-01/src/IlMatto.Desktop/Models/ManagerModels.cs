...
public sealed class ManagerMainAgentBinding
{
    ...
    public string DisplayName => Provider == "openai_compatible"
        ? "API Manager"
        : string.Equals(Transport, "sdk", StringComparison.OrdinalIgnoreCase) ? "Antigravity SDK" : "Antigravity CLI";
}
...
{
    public ManagerChatEntry(string role, string source, string text, DateTime? createdAt = null, IEnumerable<ManagerImageAttachment>? attachments = null)
    {
        Role = role;
        Source = source;
        Text = text;
    ...
    public string AvatarText => IsUser ? "你" : Source is "pi" or "codex" ? "Pi" : "AI";
